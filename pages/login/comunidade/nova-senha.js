...
function CommunityForgotPassword (props) {
  ...
  const requestNewPassword = async values => {
    try {
      setLoading(true)
      const response = await Api.forgotCommunityPassword({ ...values })
                ...
                >
                  {({ errors, touched }) => (
                    <Form>
                      ...
                      <S.ButtonsWrapper>
                        <E.CustomButton width="42%" type="submit" color="info">Solicitar nova senha</E.CustomButton>
                        <E.CustomButton tag={B.A} href="/login/comunidade" width="54%" color="danger">Voltar</E.CustomButton>
                      </S.ButtonsWrapper>
                    </Form>
                  )}
                </Formik>
...
CommunityForgotPassword.propTypes = {
